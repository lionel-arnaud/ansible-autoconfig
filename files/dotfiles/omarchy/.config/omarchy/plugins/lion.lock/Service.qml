...
Item {
  ...
  Process {
    id: wakeProcess
    // The packaged helper restores clamshell state but can leave DPMS off on
    // DP-3 after resume. Re-enable outputs after its display configuration work.
    command: ["bash", "-c", "omarchy-system-wake; hyprctl dispatch 'hl.dsp.dpms({ action = \"on\" })'"]
  }
